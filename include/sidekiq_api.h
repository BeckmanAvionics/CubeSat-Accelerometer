 ...
 *
 * Sidekiq is a software defined radio card in a MiniPCIe, M.2, or VITA 57.1
 * FPGA Mezzanine Card (FMC) form factor (Sidekiq X2 and X4).  It utilizes an
 * RFIC, which provides the complete RF front end & baseband analog & A/D and
 * D/A converters.  An on-board FPGA then provides timestamping/buffering, along
 * with optional signal processing.
 *
 * For the MiniPCIe and M.2 form factors, a single lane (x1) PCIe interface in
 * the FPGA provides a transport path between the host system and Sidekiq, which
 * is used for streaming data between the host and Sidekiq, as well as for
 * command/control of Sidekiq through a register interface.  A USB 2.0 high
 * speed interface is also included in Sidekiq, which is used to provide a path
 * for re-programming the FPGA bitstream.  This USB interface can also be used
 * by the host for streaming of data and command/control of the card for host
 * systems that include a MiniPCIe card slot but only wire up the USB 2.0 pins.
 * See the <a href="http://www.epiqsolutions.com/sidekiq/">Epiq Solutions
 * Website</a> for more details.
 *
 * The Sidekiq Z2 is offered in a MiniPCIe form factor but uses a USB 2.0 high
 * speed interface as a transport between the host system and the Zynq FPGA.
 * See <a href="https://epiqsolutions.com/modules/sidekiq-z2/">Sidekiq Z2</a>
 * for more details.
 *
 * The VITA 57.1 FMC form factor can be used in conjunction with compliant FPGA
 * carrier boards to provide a user with access to IQ samples and command /
 * control.  See <a href="http://www.epiqsolutions.com/sidekiq-x2/">Sidekiq
 * X2</a> and <a href="https://epiqsolutions.com/modules/sidekiq-x4/">Sidekiq
 * X4</a> for more details.
 *
 ...
 *  - PCIe x1 (2.5 GT/s) interface to host + USB 2.0 Hi-Speed interface
 *  - Integrated FPGA for custom signal processing and PCIe data transport to host
 *  - Integrated temperature sensor + accelerometer
 *
 * The following list enumerates the features of Sidekiq (M.2 card form factor):
 *  - Flexible RF front end supports two operating modes:
 *    - Two RF receiver + two RF transmitter (2x2 MIMO)
 *    - One RF receiver + one RF transmitter (separate LOs)
 *  - RF tuning range from 70 MHz to 6 GHz
 *  - Up to 50 MHz RF bandwidth per channel (min sample rate: 233 Ksps, max sample rate: 61.44 Msps)
 *  - PCIe x2 (5.0GT/s) interface to host + USB 2.0 Hi-Speed interface
 *  - Integrated FPGA for custom signal processing and PCIe data transport to host
 *  - Integrated temperature sensor + accelerometer
 *
 ...
 *  - 10MHz + PPS sync inputs
 *
 * Documentation for the primary Sidekiq API exists in these two files:
 *  - sidekiq_api.h
 *  - sidekiq_types.h
 *
 * Documentation for the custom transport developers, the Sidekiq Transport API, exists in these two files:
 *  - sidekiq_xport_api.h
 ...
 * ADXL346</a>) for miniPCIe Sidekiq and M.2 Sidekiq products.  The Sidekiq Z2 product uses <a
...
#define LIBSIDEKIQ_VERSION_MINOR 9

/** @brief Patch version number for libsidekiq */
#define LIBSIDEKIQ_VERSION_PATCH 3

/** @brief Label version for libsidekiq */
#ifdef _WIN32
#define LIBSIDEKIQ_VERSION_LABEL "-win64"
#else
...
/** @brief When running in packed mode, every 3 words of data contain 4 samples.
    ::SKIQ_NUM_WORDS_IN_PACKED_BLOCK converts from the number of samples to the
    number of words need for to hold that sample @hideinitializer */
#define SKIQ_NUM_WORDS_IN_PACKED_BLOCK(num_packed_samples) ( ( (num_packed_samples) * 3) / 4 )
...
#define SKIQ_END_USER_FPGA_REG_ADDR 0x000089FF
...
/** @brief ::SKIQ_MIN_SAMPLE_RATE is the minimum Rx/Tx sample rate that can
    ...
    radio configuration, refer to skiq_read_min_sample_rate(). */
...
/** @brief ::SKIQ_MAX_SAMPLE_RATE is the maximum Rx/Tx sample rate that can
    ...
    radio configuration, refer to skiq_read_max_sample_rate(). */
...
{
    return (skiq_tx_block_t *)calloc(1, SKIQ_TX_HEADER_SIZE_IN_BYTES + data_size_in_bytes);
}
...
/** The skiq_get_cards() function is responsible for generating a list
    of valid Sidekiq card numbers for the transport specified.  Return of the
    ...
    @note Can be called before skiq_init()
    ...
    @param[out] p_cards pointer to where to store the card numbers of the
...
/** The skiq_get_card_from_serial_string() function is responsible for obtaining
    the Sidekiq card number for the specified serial number.

    @ingroup cardfunctions
    @see skiq_read_serial_string

    @param[in] p_serial_num serial number of Sidekiq card
    @param[out] p_card pointer to where to store the corresponding card number
...
/** The skiq_init() function is responsible for performing all initialization
    ...
    @retval -EINVAL if one of the specified card numbers is out of range or refers to a
                    non-existent card
*/
...
/** The skiq_enable_cards_by_serial_str() function is responsible for performing all initialization
    ...
    @param[in]  level           the transport functionality level of initialization that is
      ...
      - ::skiq_xport_init_level_full - Same as ::skiq_xport_init_level_basic and
          perform the complete bring up of all hardware (most applications
          concerned with sending/receiving RF will use this)
    @param[out] p_card_nums     pointer to the list of Sidekiq card numbers corresponding with
...
/** The skiq_init_by_serial_str() function is identical to skiq_init() except a
    list of serial numbers can be requested instead of card numbers.
...
/** The skiq_is_xport_avail() function is responsible for determining if the
    requested transport type is available for the card number specified.
...
/** The skiq_is_card_avail() function is responsible for determining if the
    ...
    @retval -ERANGE if the specified card number exceeds the maximum (::SKIQ_MAX_NUM_CARDS)
    @retval -ENODEV if a card was not detected at the specified card number
...
/*****************************************************************************/
/** The skiq_read_last_1pps_timestamp() function is responsible for returning
    the RF and system timestamps of when the last @ref ppsfunctions "1PPS"
    timestamp occurred.

    @ingroup ppsfunctions

    @param[in] card card number of the Sidekiq of interest
    @param[out] p_rf_timestamp  a pointer to a uint64_t where the value of the RF
    timestamp when the last @ref ppsfunctions "1PPS" occurred
    @param[out] p_sys_timestamp  a pointer to a uint64_t where the value of the sys
    timestamp when the last @ref ppsfunctions "1PPS" occurred
    @return int32_t  status where 0=success, anything else is an error
*/
...
/*****************************************************************************/
/** The skiq_write_rx_gain() function is responsible for setting the overall
    gain of the Rx lineup for the specified receiver by means of providing an
    index that maps to a specified gain.  The gain index value is a direct
    index into the gain table of the radio.  The mapping of gain
    index to gain in dB is dependent on the RFIC used by the product.

    For Sidekiq mPCIe / m.2 / Z2, each increment of the gain index value results
    in approximately 1 dB of gain, with approximately 76 dB of total gain available.
    ...
    - For Sidekiq X2, the A1 (Rx1) & A2 (Rx2) receivers have approximately 30 dB of total
    gain available, where an increment of 1 in the gain index value results in approximately
    0.5 dB increase.  The B1 (ObsRx) receiver has approximately 18 dB of total gain 
    available, where an increment of 1 in the gain index value results in approximately 
    1 dB increase in gain.  For details on the gain table available, refer to the 
    "Gain Table" section on p. 120 of the AD9371 User Guide (UG-992)

    - For Sidekiq X4, each receiver has 30 dB of total gain available, where an increment
    of 1 in the gain index results in approximately 0.5 dB increase.  For details on
    the receiver datapath and gain control blocks, refer to the "Receiver Datapath"
    on p.125 of the ADRV9008-1/ADRV9008-2/ADRV9009 Hardware Reference Manual (UG-1295)
...
/*****************************************************************************/
/** The skiq_read_rx_gain() function is responsible for retrieving the
    current gain index for the specified Rx interface.  The gain index value
    is a direct index into the gain table of the radio.  The mapping of gain
    index to gain in dB is dependent on the RFIC used by the product.
...
/** The skiq_read_tx_num_late_timestamps() function reads the current number of
    ...
    value is reset only when calling skiq_start_tx_streaming().
...
/** The skiq_write_accel_reg() function provides generic write access to the
    ...
    @param[in] card card number of the Sidekiq of interest
    @param[in] reg regiser address to access
...
/*****************************************************************************/
/** The skiq_read_accel_reg() function provides generic read acess to the
...
/** The skiq_write_tcvcxo_warp_voltage() function is responsible for setting a
    new warp value for the reference clock oscillator.  A 10-bit DAC is
    controlled by this function and the DAC can generate voltage between 0.75
    and 2.25V.  Valid warp voltages for the ref clock oscillator are from 0.75 -
    2.25V (which corresponds to DAC values between 0-1023).
    ...
    @param[in] card card number of the Sidekiq of interest
    @param[in] warp_voltage a 16-bit value corresponding to the desired DAC
...
/** The skiq_read_default_txcvxo_warp_voltage() function is responsible for
    ...
    warp voltage value is automatically loaded during skiq_init() or
    skiq_init_by_serial_str() unless a user value is defined in which case the
    user value is loaded during initialization.
...
/** The skiq_read_user_txcvxo_warp_voltage() function is responsible for
    ...
    by the user and is automatically loaded during a call to skiq_init() or
    skiq_init_by_serial_str().  This value takes presedence over the default
    value loaded by the factory.
...
/** The skiq_write_user_txcvxo_warp_voltage() function configures the
    ...
    and is automatically loaded during a call to skiq_init() or
    skiq_init_by_serial_str().  This value takes presedence over the default
    value loaded by the factory.
...
/** The skiq_write_rx_data_src() function is responsible for setting the data
    souce for the Rx interface.  This is typically complex I/Q samples, but can
...
/** The skiq_write_rx_stream_mode() function is responsible for setting the
    ...
    @attention If the receive stream mode is set to
    ::skiq_rx_stream_mode_low_latency and an incompatible FPGA bitstream is then
    loaded via skiq_prog_fpga_from_file() or skiq_prog_fpga_from_flash(), the
    mode will automatically revert to ::skiq_rx_stream_mode_high_tput without
    warning.
...
/*****************************************************************************/
/** The skiq_read_curr_rx_timestamp() function is responsible for retrieving the
    currently set value for the timestamp (i.e., free-running counter) of the
    specified interface handle.  This timestamp is maintained by the FPGA and is
    shared across each RFIC regardless of the Rx or Tx interface.
    ...
    @param[in] card card number of the Sidekiq of interest
    @param[in] hdl [::skiq_rx_hdl_t]  the handle of the interface for which the current timestamp
    is being read
...
/** The skiq_prog_rfic_from_file() function is responsible for pushing
    ...
    part of skiq_init() or skiq_init_by_serial_str().
...
/*****************************************************************************/
/** The skiq_prog_fpga_from_file() function is responsible for programming
    the FPGA with an already opened bitstream file. This allows
    libsidekiq-based apps to reprogram the FPGA at run-time if needed.

    @note After successful reprogramming is complete, all RX interfaces are
    reset to the idle (not streaming) state.
...
/*****************************************************************************/
/** The skiq_prog_fpga_from_flash() function is responsible for programming the
    FPGA from the image stored in flash. This allows libsidekiq-based apps to
    reprogram the FPGA at run-time if needed.

    @note after successful reprogramming is complete, all RX interfaces
    are reset to the idle (not streaming) state.
...
/*****************************************************************************/
/** The skiq_save_fpga_config_to_flash() function stores a FPGA bitstream into
    flash memory, allowing it to be automatically loaded on power cycle or
    calling skiq_prog_fpga_from_flash().

    @note after successful reprogramming is complete, Sidekiq is restored to
    default configuration and all Rx interfaces are idled (not streaming).
...
/*****************************************************************************/
/** The skiq_verify_fpga_config_from_flash() function verifies the contents of
    flash memory against a given file. This can be used to validate that a given
    FPGA bitstream is accurately stored within flash memory.
...
/*****************************************************************************/
/** The skiq_read_max_sample_rate() function returns the minimum sample rate
...
/** The skiq_write_rfic_rx_fir_coeffs() function allows the coefficients of the
    ...
    determined by the sample rate and can be queiried using
...
/** The skiq_register_logging() function allows a custom logging handler to be
    registered.  The priority (as defined by syslog.h, 0:7) and the logging
